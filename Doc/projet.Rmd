...
On présente également la décomposition biais-variance pour les jeux d'entraîenemnt et de test :


```{r}

res2 <- data.table(ech = c("Entraînement", "Test"),
    rmse = c(mixed_model$rmse_train,mixed_model$rmse_test),
   biais = c(mixed_model$bias_train, mixed_model$bias_test),
   var = c(mixed_model$var_train, mixed_model$var_test),
   var_irr = c(mixed_model$rmse_train - mixed_model$bias_train - mixed_model$var_train, mixed_model$rmse_test - mixed_model$bias_test - mixed_model$var_test)
)
...
res2 %>% 
   kbl(
      align = "c", 
      digits = 3,
      col.names = c("Echantillon", "RMSE", "Biais", "Variance", "Variance irréductible"),
      booktabs = TRUE, 
      escape = FALSE
   ) %>% 
   kable_classic(
...
Comme indiqué dans le tableau ci-dessus, l'Erreur Quadratique Moyenne du modèle linéaire mixte vaut 0.194 sur le jeu d'entraînement et 0.198 sur le jeu de test. Ainsi, la différence entre les EQM sur les jeux d'entraînement et de test est faible, ce qui suggère que le modèle généralise très bien sur de nouvelles données.

Après avoir décomposé l'EQM, on s'aperçoit que le biais est très faible : de l'ordre de 0.037 pour l'échantillon d'entraînement, et de 0.039 pour l'échantillon de test. Ainsi, on peut en conclure que le modèle ajusté est non biaisé pour nos données.
