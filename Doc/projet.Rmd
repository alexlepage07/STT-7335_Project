...
pca1 <- readRDS(file.path(path_obj, "s4_pca1.rds"))
pca2 <- readRDS(file.path(path_obj, "s4_pca2.rds"))
...
Nous faisons, donc, une analyse des composantes principales avec la fonction `PCA` de la librairie `FactoMineR`. Celle-ci standardise, par défaut, nos variables. L'objectif de cette analyse des composantes principales est de faire une première sélection des variables pas trop sévère et, surtout, retirer certaines variables fortement linéairement corrélées.
...
graph_pca_var_cum(pca1, threshold = 0.9)
...
graph_pca(pca1, nb_var = 10)


```

<span style="color:red">Ajouter un tableau avec les corrélations.</span>

Nous remarquons à partir de ce graphique que nous avons beaucoup de variable en double qui sont grandement corrélée, quasi parfaitement. Un exemple est la variable `pre_mm_uyr` et `pre_mm_cyr` qui sont les précipitations printanières moyennes (en mm), respectivement, dans le bassin versant et dans le bassin versant total en amont du point d'écoulement. Nous retirons ces doublons de variables qui ont vraiment peu de différence. Nous réduisons le nombre de variables de notre jeu de données de ~50 à ~35.

On affiche de nouveau ce graphique :

```{r}


# Afficher les deux premières composantes
graph_pca(pca2, nb_var = 10)
