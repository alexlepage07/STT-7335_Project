...
mixed_model <- readRDS(file.path(path_obj, "s8_mixed_effect.rds"))

river_lasso_dt <- readRDS(file.path(path_data, "s7_donnees_lasso_var_sel.rds"))
...
Selon Ressource naturelle Canada, les inondations représentent le type de catastrophes naturelles le plus coûteux au Canada[^1]. Afin de faire face à ce type de risque il va de soit de comprendre le phénomène et d'être en mesure d'identifier les zones à risque en produisant des cartes d'inondation. Pour les secteurs ayant des stations hydrologiques, la tâche est relativement simple. Cependant, lorsque l'on veut extrapoler sur des bassins versants non jaugés, la tâche se complexifie.

[^1]: [Ressource Naturelle Canada](https://natural-resources.canada.ca/science-and-data/science-and-research/natural-hazards/flood-mapping/24223)
...
Comme les rivières sont inter-connectées les unes aux autres, une forte dépendance lie le débit de chacune d'entre elles. Pour tenir compte de cette structure de dépendance, nous proposons un modèle linéaire mixte. Par ailleurs, comme ce projet est purement exploratoire et vise uniquement à tester le pouvoir prédictif d'un éventuel modèle de débit, nous ne cherchons pas à extrapoler sur de nouveaux bassins versants ou pour une période de temps ultérieure. En revanche, calculer un intervalle de confiance sur les débits prédits permettra d'avoir une appréciation substantielle de la performance du modèle puisque nous pourrons comparer ceux-ci aux débits moyens historiques fournis dans le jeu de données. Cette analyse viendra compléter la métrique de performance commune qu'est l'erreur quadratique moyenne (EQM). À noter que, bien que [Chaduri et al., 2021](@chaudhuri2021inundated) fait ses prédictions sur la médiane, nous nous concentrerons sur la moyenne des débits annuels. La raison étant que les modèles mixtes standards sont fait pour estimer la moyenne d'une distribution normale et que les données de RiverAtlas ne fournissent que cette information et non pas les quantiles ou toutes les observations historiques.

## Préparation et nettoyage des données

Nous utilisons les données de l'HydroAtlas[^2], provenant du site de [HydroSheds](https://www.hydrosheds.org/hydroatlas) pour tenter de prédire le débit moyen des rivières pour un bassin versant donné. Nous focalisons nos efforts dans la province du Québec en prenant les données nord-américaines et en rognant les fichiers de formes (shapefiles) autour de la province. Le jeu de données ainsi créé contient 162 507 observations et 295 variables disponibles. Chaque observations du jeu de données correspond à un bassin versant et les variables représentent des données utiles pour effectuer des études hydrologiques, telles que les précipitations, le volume d'eau en amont, la pente hydrique, etc. La variable d'intérêt est la suivante: `dis_m3_pyr`. Celle-ci représente la moyenne annuelle des débits d'eau en . Le débit minimal observé est de 0.075 et il peut atteindre des valeurs aussi élevées que 12 343, avec une moyenne de 25. Ces statistiques témoignent de la forte asymétrie existant dans la distribution probabiliste du débit d'eau.

[^2]: Pour en apprendre d'avantage sur les données utilisées, consultez le site internet d'HydoSheds à l'adresse suivante: <https://data.hydrosheds.org/file/technical-documentation/HydroATLAS_TechDoc_v10_1.pdf>
...
[^3]: Les variables constituant les données sont décrites à cette adresse: <https://data.hydrosheds.org/file/technical-documentation/RiverATLAS_Catalog_v10.pdf>

Parmi celles-ci, il y en a 4 qui sont catégorielles: classification du terrain, du milieu humide, des types de roche et la classification hydrologique du bassin versant. Puis, il y a 41 variables numériques correspondant à des pourcentages. Finalement, les 11 variables restantes sont numériques. Certaines d'entre elles, comme l'aire du bassin ou le volume hydrique, contiennent des valeurs extrêmes.
...
Les données manquantes dans un jeu de données sont chose commune et peuvent être issues de diverses causes comme des capteurs défectueux, d'oublis lors de la collecte des données ou, encore, de mesures ajoutées en cours de route d'un projet. Notre jeu de données ne fait pas exception : nous avons des données manquantes dénotées par des *-999*'s. Pour faire le portrait des données manquantes de notre jeu de données, le Tableau 2.1.1 (Annexe A1) expose en détail les variables pour lesquelles nous avons des données manquantes et le nombre/pourcentage d'observations qui sont touchées. La variable la plus touchée est la classification des zones humides, qui contient 44% de données manquantes. Les autres variables touchées contiennent moins de 1% de données manquantes.

#### Premiers traitements

L'acronyme *cav* désigne le bassin versant observé (*Catchment*), tandis que *uav* désigne une aggrégation des bassins en amont (*upstream*). Nous remarquons qu'une variable ressort du lot avec un pourcentage de près de 44% de valeurs manquantes soit celle de la classification des zones humides. Or, dans la description des variables de notre jeu de données, [*RiverATLAS Attributes*](https://data.hydrosheds.org/file/technical-documentation/RiverATLAS_Catalog_v10.pdf), on y spécifie que les *-999*'s sont, en fait, les endroits n'ayant pas de zone humide ce qui explique pourquoi elles ne sont pas classifiées. Cette variable contient 12 classes numérotées de 1 à 12. Pour traiter les non-réponses de cette variable, nous y ajoutons un 13e classe qui équivaut à *aucune zone humide*. Nous remplaçons, donc, les valeurs *-999* par un facteur correspondant à *0*.
...
D'une part, notons que chaque variable est mesurée pour le bassin observé ( *cav* ) et pour les bassins en amont ( *uav* ). Or, dans ces cas, le patron de non-réponse est très similaire. En effet, si une variable est manquante pour tous les bassins en amont, elle le sera assurément pour le bassin présent. Par ailleurs, notamment grâce au Tableau 2.1.2 (Annexe A1), il est intéressant d'observer que les variables *cav* et *uav* sont linéairement très corrélées.

Étant donné cette forte corrélation linéaire, et considérant qu'il y a nettement moins de valeurs manquantes pour les variables portant le suffixe *uav* que *cav*, nous retirons ces dernières.

#### Pourcentage par patrons de non-réponse

Nous voyons dans le Tableau 2.1.3 (Annexe A1) le seul patron de non-réponse de notre jeu de données ainsi que la proportion des observations qui y est associé. Celui-ci est composé des variables de la composition du sol et il représente 0.06% de nos données. À noter que chacune de ces quatre variables proviennent d'une même source de données: [SoilGrids1km; Hengl et al. 2014](https://www.isric.org/explore/soilgrids).
...
Nous faisons une analyse préliminaire pour évaluer si les données sont manquantes de façon complètement aléatoire. Pour ce faire, nous commençons à comparer les graphique de la distribution de variables explicatives pour les observations qui ne font pas parties du patron (`patron = 0`) au graphique de la même variable mais avec les observations qui font partie de ce patron (`patron = 1`). Nous montrons en annexe A1 quelques exemples de variables dont la distribution changent en fonction de si les observations font parties du patron ou non (débit d'eau moyen, proportion occupée par des forêts...) ce qui suggère que nous ne sommes pas en présence de valeurs manquantes complètement aléatoire.
...
Pour poursuivre notre analyse, nous exposons dans le tableau 2.1.4 (Annexe A1) l'ensemble des *test-t*'s possibles pour comparer la moyenne des autres variables pour les observations faisant parties du patron et celle pour les observations ne faisant pas parties du patron.
...
Nous remarquons que 39 tests ont un seuil observé en dessous de 5%. Cela signifie que pour ces tests la moyenne de la variable évaluée est significativement différente en présence ou non du patron de non-réponse.

En revanche, faire autant de *test-t*'s a ses limites. En effet, puisque nous faisons au total 44, nous avons le problème de comparaisons multiples qui implique que, même si toutes les moyennes étaient réellement égales, une partie des tests seraient rejetée. Or, dans notre cas, une très grande partie le sont ce qui appuie notre hypothèse comme quoi les données ne sont pas manquantes complètement aléatoirement.

Pour valider nos analyses préliminaires, nous utilisons le test de *Little*, qui calcule une statistique telle que : $$d^2 = \sum_{j = 1}^J (\hat{\mu}_j - \hat{\mu}_j^{ML})\hat\Sigma_j^{-1}(\hat{\mu}_j - \hat{\mu}_j^{ML})$$ Si les données sont manquantes de façon complètement aléatoire, nous aurons que $$d^2 \sim \chi_{\sum_{j=1}^J k_j-k}.$$

En utilisant la formule `naniar::mcar_test`, nous obtenons que le seuil observé est de 0 qui confirme notre hypothèse. On rejette, donc, l'hypothèse $H_0 :$ Données manquantes avec un mécanisme *MCAR*.

#### Conclusion de l'analyse

En conclusion, comme la non-réponse pouvait être prédite selon la variable de pourcentage de couverture des lacs dans le bassin (`lka_pc_cse`) et que ce patron était associé à des valeurs significativement différentes pour les autres variables, on peut affirmer que le patron de non-réponse est MAR.

Évidemment, le fait de retirer des données MAR peut introduire un biais. Néanmoins, l'objectif de ce travail est de prédire le débit d'eau des rivières. Comme il n'y a pas de sens à calculer un débit d'eau dans un lac, nous allons simplement retirer ces observations du jeu de données.
...
Pour la première composante principale Les variables corrélées positivement sont `pre_nn_xxx` (poids de 0,38 qui représente la quantité de précipitation), `gwt_cn_cav` (0,18 est la profondeur de la nappe phréatique) alors que `slp_dg_cav`/`uav` (0,14 correspond à la pente du terrain).
...
Pour la seconde composante principale Les variables qui présentent une corrélation positive sont `rev_nc_usu` (0,34 le volume du réservoir), `ria_ha_csu`/`usu` (0,40 la surface de la rivière) et `riv_tc_csu`/`usu` (0,45 le volume de la rivière).
...
Pour la troisième composante principale Les variables avec une corrélation positive sont `gwt_cn_cav` (0,36 profondeur de la nappe phréatique), `slp_dg_uav`/`cav` (0,41 est la pente du terrain) tandis que `sgr_dk_rad` (0,26 est le gradient de l'affluent) et finalement `ero_hk_cav`/`uav` (0,40 correspond à l'érosion).
...
Dans notre cas, nous avons identifié qu'il existe une dépendance basée sur la localisation, mais plutôt au travers les rivières et dans quel bassin elles se déversent. Ainsi, deux rivières qui se déversent dans le même bassin seront considérées dépendantes au travers les effets aléatoires de notre modèle. Ainsi, on obtient :
...
Avant de faire une présélection des variables à partir de la forêt aléatoire, nous venons retirer les variables qui sont fortement corrélées. En effet, l'importance des variables peut être affecté par la multicolinéarité. Dans l'article *Correlation and variable importance in random forests*[^8], on y souligne le risque dans un contexte de multicolinéarité d'obtenir des résultats d'importance des variables instables : une petite modification du jeu d'entraînement peut entraîner une changement complet des variables dites importantes. Dans notre jeu de données, certaines variables sont très corrélées. Nous avons, notamment, des mesures qui ont été prises à différents endroits du bassin versant qui ont pratiquement les mêmes valeurs. Nous avons, également, la situation où nous avons des prises de mesure pour différents mois subséquents qui sont quasi-parfaitement corrélés. De ce fait, nous retirons certaines variables basé sur un diagnostique de multicolinéarité. Les détails de ce diagnostique (chaque itération) se retrouve en [annexe](#multicolinearity). Ce diagnostique fait en sorte que nous retirons les variables suivantes : `r tolower(paste(translate_var(ind_cond$removed_vars, suffixe = FALSE), collapse = ", "))`.

[^8]: Idem

#### Forêt aléatoire

La forêt aléatoire entraînée pour la présélection a été construite à partir de la librairie R *ranger* (en [annexe](#random_forest) se retrouve les détails sur la méthodologie et les performances du modèl). L'importance des variables est obtenu à partir de la permutation des variables explicatives du jeu de données *Out of the Bag* (OOB). Comme décrit dans *ELS*[^9], l'idée générale est que, pour chaque arbre, nous avons un jeu de données laissé à part (OOB) que nous utilisons pour calculer, pour chaque variable explicative, l'augmentation de l'erreur ou la diminution de l'exactitude entre les prédictions du jeu de données non-modifié et celles du jeu de données où nous permutons les valeurs de la variable évaluée. À chaque itération, en venant calculer l'impact sur un jeu de données indépendant des données vues par l'arbre entraîné, nous avons un portrait adéquat des variables importantes en généralisation, et non, importantes pour entraîner le modèle. Dans la librairie *ranger*, l'importance des variables est calculé à partir de la méthode décrite dans l'article *A computationally fast variable importance test for random forests for high-dimensional data. Adv Data Anal Classif*[^10].

[^9]: The elements of statistical learning : data mining, inference, and prediction, p. 593

[^10]: Janitza, S., Celik, E. & Boulesteix, A.-L., (2015). A computationally fast variable importance test for random forests for high-dimensional data. Adv Data Anal Classif doi: 10.1007/s116340160276- 4
...
Nous avons entraîné un arbre de décision pour sélectionner les interactions les plus importantes à tester. Pour se faire, nous avons fixé l'arbre de décision a une profondeur de 5 pour qu'il reste interprétable. Puis, nous avons optimisé le reste des hyper-paramètres par validation croisée sur le jeu d'entraînement (pour plus de détails sur la méthodologie et les performances du modèle, voir la section en [annexe](#decision_tree)). Nous voyons que l'arbre dans les premiers niveaux utilise très fréquemment la variable `r translate_var("riv_tc", suffixe = FALSE)` et, dans les niveaux suivants, l'`r translate_var("ria_ha", suffixe = FALSE)` ressort souvent. Nous allons tester dans le modèle *Lasso* l'interaction entre ces deux variables. Des interactions qui arrivent qu'une fois que nous allons tester, également, sont :
...
Nous utilisons un modèle *Lasso* pour compléter notre sélection de variables. Ce modèle, ayant comme caractéristique d'utiliser une régularisation avec la norme L1, permet de réduire les poids des variables les moins utiles à zéro. Ce faisant, il permet d'effectuer une sélection de variables. Par ailleurs, le modèle a l'avantage de gérer les situations où nous avons deux variables fortement corrélées. En effet, dans une régression normale avec deux variables très corrélées, l'un pourrait recevoir un coefficient positif élevé et l'autre un coefficient négatif pour capturer l'effet commun aux deux variables.[^11]. Dans un modèle *Lasso*, le coefficient de l'une de ces 2 variables sera mis à 0 éliminant l'une des variables et gérant la situation de colinéarité. En [annexe](#lasso), nous décrivons plus en détail ce modèle, notre méthodologie pour entraîner le modèle et la performance de ce dernier.

[^11]: he elements of statistical learning : data mining, inference, and prediction, p. 63

Avant de faire ce modèle, nous avons fait une modèle additif généralisé (*GAM*) pour ajouter au modèle *Lasso* certaines transformations polynomiales. Le modèle Lasso a conservé 3 transformations polynomiales de degré 2 pour les variables : `r tolower(paste(translate_var(lasso_model$var_poly), collapse = ", "))`. Pour plus de détails, se référer à l'[annexe](#lasso). De plus, le modèle a sélectionné l'interaction : `r translate_var("riv_tc", suffixe = FALSE)` x l'`r translate_var("ria_ha", suffixe = FALSE)`.

Voici, les résultats de la validation croisée pour obtenir le $\lambda$ optimal pour minimiser l'EQM ainsi que les variables sélectionnées par le modèle avec un tel $\lambda$.
...
Le RMSE obtenu pour notre jeu de test et, en comparaison, notre jeu d'entraînement :

```{r}
res <- data.table(

   rmse_train = mixed_model$rmse_train,

   rmse_val = mixed_model$rmse_test
...
res %>% 
   ...
   kbl(
      ...
      col.names = c("RMSE (entr.)", "RMSE (test.)"),
...
On présente l'Erreur Quadratique Moyenne, avec le biais :

```{r}
res2 <- data.table(

    eqm = (MLmetrics::RMSE(predict(mixed_model$final_model, river_lasso_dt, allow.new.levels = TRUE), river_lasso_dt$dis_m3_pyr))^2,

   biais = mean(river_lasso_dt$dis_m3_pyr - predict(mixed_model$final_model, river_lasso_dt, allow.new.levels = TRUE))^2
...
Comme indiqué dans le tableau ci-dessus, la racine carrée de l'Erreur Quadratique Moyenne (RMSE) du modèle linéaire mixte vaut 0.194 sur le jeu d'entraînement et 0.198 sur le jeu de test. Ainsi, la différence entre les RMSE sur les jeux d'entraînement et de test est faible, ce qui suggère que le modèle généralise très bien sur de nouvelles données.

Après avoir affiché le biais, on s'aperçoit qu'il vaut approximatiement 0. Ainsi, on peut en conclure que le modèle ajusté est non biaisé pour nos données.

A titre de comparaison, la forêt aléatoire ajustée plus haut possédait une EQM de 0.139 sur le jeu d'entraînement, et de 0.162 sur le jeu de test. Ainsi, bien que le modèle linéaire mixte généralise mieux que la forêt aléatoire, il semble globalement moins bien performer. Cela pourrait s'expliquer par le fait que le modèle linéaire mixte ne prend pas en compte les interactions non-linéaires entre les variables, contrairement à la forêt aléatoire. De plus, il se peut que la différence des variables prises en compte dans les deux modèles joue un rôle dans la différence d'EQM de ces derniers. En effet, il est possible que certaines variables non sélectionnées par le LASSO (mais incluses à l'étape de la forêt aléatoire) car elles avaient un effet principal non significatifs soient pertinentes de par leur interaction avec d'autres variables du modèle.

## Enjeux éthiques

Comme l'objectif est de prédire des débits d'eau, il y a très peu aspects éthiques liés au projet.

Néanmoins, si l'aboutissement de notre travail pouvait servir au milieu de l'assurance, cela contribuerait à préciser la tarification des couvertures pour le risque d'inondation. Conséquemment, on pourrait observer une prime d'assurance plus basse à certains endroits et plus hautes à d'autres. L'enjeu éthique, dans ce cas, se refléterait dans le risque d'anti-sélection (équité) puisque la prime deviendrait juste trop chère pour les personnes les plus à risque. Or, dans les régions éloignées (précisément les endroits qui justifient l'utilisation de notre projet), les gens ont des revenus plus faibles qu'à proximité des grands centres. On parle entre autre des populations autochtones. Ces personnes ainsi exposées ne sont pas en mesure de payer une prime élevée et il peut parfois être impossible de relocaliser leur domicile.

En revanche, sur ce point, le modèle n'est pas responsable de leur forte exposition au risque, mais seulement de l'habilité à identifier les zones de risque. Finalement, si nous sommes en mesure de bien identifier les zones de risque, il sera possible de travailler en partenariat avec les municipalités et avec les instances gouvernementales pour protéger ces personnes vulnérable par des subventions pour la relocalisation ou pour l'adaptation au risque. En somme, ce modèle peut apporter à la fois un problème et une solution. Il s'agira alors de savoir en tirer le meilleur usage.

## Recommandation du modèle?

En somme, nous avons démontré que certaines variables peuvent effectivement être utiles pour prédire les débits d'eau annuels moyens. Cependant, étant données la forme des termes d'erreurs et l'erreur quadratique moyen plus élevé avec le modèle final qu'avec la forêt aléatoire utilisée pour la pré-sélection des variables explicatives, nous concluons qu'un modèle linéaire n'est pas le choix le plus optimal pour capturer les interactions entre les différentes variables du modèle. À ce titre, il aurait sans doute été préférable d'utiliser une forêt aléatoire mixte (voir [Hajjem et al., 2014](@mixedRandomForest2014)) qui comporterait des effets aléatoires, au même titre que le modèle mixte proposé.

Également, comme il est mentionné en introduction, ce modèle, tel qu'il est présenté, n'est pas utile pour créer des cartes d'inondation. Afin de pouvoir définir des zones de risque, il faudrait pouvoir évaluer des quantiles pour des périodes de retour. Une option à envisager serait d'extraire des données de stations hydrologiques[^12], puis il deviendrait envisageable d'apparier les observations de ces stations aux variables explicatives utilisées dans ce projet. Avec un tel jeu de données, il pourrait alors être possible d'entraîner un modèle de régression par quantile tel que celui proposé dans [Pasche et al., 2022](@ExtremeQuantileNeuralNetwork2022). En ce qui attrait à la régression par quantile, en général, on peut se référer à [Koenker et al., 2017](@quantileRegression2017). De plus, l'idée de ce type de modèle est également applicable aux forêts aléatoire, même lorsque les variables étudiées comportent une queue de distribution très épaisse. À ce titre, voir [Gnecco et al., 2022](@ExtremeRandomForest2022)

[^12]: <https://wateroffice.ec.gc.ca/map/index_e.html?type=real_time>

Finalement, la combinaison des modèles proposés par [Hajjem et al., 2014](@mixedRandomForest2014)) et par [Gnecco et al., 2022](@ExtremeRandomForest2022) pourrait sans doute constituer une avenue optimale. Comme rien n'est encore fait à ce sujet dans la littérature, cela pourrait constituer un sujet de recherche prometteur.
...
river_dt <- setDT(readRDS(file.path(path_data, "s1_donnees_filtrees.rds")))
...
Tableau 2.1.2 Jusitifant de la forte corrélation entre les variables *cav* et *uav*
...
Tableau 2.1.4 résumant les seuils obtenus pour les t_test afin de determiner le patron de non-réponse
...
1.  Séparer le jeu de données en 2 parties (jeu d'entraînement et jeu de test)
2.  Identification des effets aléatoires à tester À partir du modèle *Lasso* qui ne contient aucun effet aléatoire, nous avons regardé les graphiques des résidus par rapport à différentes variables pour avoir un indication si des effets aléatoires seraient nécessaires, et ce, au travers la structure de dépendance basée sur la rivière commune en aval. Nous observons avec les graphiques suivants qu'une ordonnée à l'origine aléatoire ainsi que des pentes aléatoires pour les variables *ria_ha_csu* (aire de la rivière) et *riv_tc_usu* (volume de la rivière) seraient pertinentes à tester :

```{r}

mixed_model$graphs_res$res_vs_var$ria_ha_csu

```

```{r}

mixed_model$graphs_res$res_vs_var$riv_tc_usu

```

3.  Choix de la structure de covariance

Pour valider les effets aléatoires que nous suggère les graphiques, nous testons différents modèles que nous soumettons à des tests du rapport de vraisemblance pour choisir le modèle le plus complexe qui est justifié d'utiliser selon ce test :

$$H_0 = \text{Modèle simple}$$ $$H_1 = \text{Modèle plus complexe}$$ Les modèles testés sont les suivants :

1.  Modèle avec une ordonnée à l'origine aléatoire
2.  Modèle avec une ordonnée à l'origine aléatoire et une pente aléatoire pour la variable *ria_ha_csu* (sans corrélation, ce qui implique que la matrice de variance-covariance est diagonale)
3.  Modèle avec une ordonnée à l'origine aléatoire et une pente aléatoire pour la variable *ria_ha_csu* (avec corrélation, chaque combinaison d'effets aléatoires a un coefficient de corrélation différent)
4.  Modèle avec une ordonnée à l'origine aléatoire et des pentes aléatoires pour les variables *ria_ha_csu* et *riv_tc_usu* (sans corrélation)
5.  Modèle avec une ordonnée à l'origine aléatoire et des pentes aléatoires pour les variables *ria_ha_csu* et *riv_tc_usu* (avec corrélation)

```{r}

dt <- data.table(
   model = c("Modèles 1 vs 2", "Modèles 2 vs 3", "Modèle 3 vs 4", "Modèle 4 vs 5"),
   p_value = fifelse(sapply(mixed_model$rapport_vrais_test, function(x) {x$`Pr(>Chisq)`[2]}) < 0.001, "< 2.2e-16", "")
)

dt %>% 
   knitr::kable(
      col.names = c("Modèles comparés", "P-valeur"), 
      align = rep("c", 2)
   ) %>% 
...
Notre modèle final a, donc, la forme :

$$\boldsymbol{Y} = \boldsymbol{X}\boldsymbol{\beta} + \boldsymbol{Z}\boldsymbol{\gamma} + \boldsymbol{\epsilon}$$

$$\boldsymbol{\gamma}_{\text{main_riv}} = \begin{bmatrix}
\gamma_{\beta_0\text{, main_riv1}}\\
\gamma_{\beta_1\text{, main_riv1}}\\
\gamma_{\beta_2\text{, main_riv1}}\\
\gamma_{\beta_0\text{, main_riv2}}\\
... \\
\gamma_{\beta_2\text{, main_riv3844}}\\
\end{bmatrix}_{11532 \times 1}$$

Où

-   $\beta_0$ est lié à l'ordonnée à l'origine
-   $\beta_1$ est lié la pente de la variable *ria_ha_csu*
-   $\beta_2$ est lié à la pente de la variable *riv_tc_usu*

Ces effets aléatoires ont une matrice de variance-covariance telle que : $$\begin{bmatrix}
\sigma^2_{\beta_0}~~~\rho_{\beta_0, \beta_1}\sigma_{\beta_0}\sigma_{\beta_1}~~~\rho_{\beta_0, \beta_2}\sigma_{\beta_1}\sigma_{\beta_2}\\
\rho_{\beta_0, \beta_1}\sigma_{\beta_0}\sigma_{\beta_1}~~~\sigma^2_{\beta_1}~~~\rho_{\beta_1, \beta_2}\sigma_{\beta_1}\sigma_{\beta_2}\\
\rho_{\beta_3, \beta_0}\sigma_{\beta_3}\sigma_{\beta_0}~~~\rho_{\beta_3, \beta_2}\sigma_{\beta_3}\sigma_{\beta_2}~~~\sigma^2_{\beta_3}
\end{bmatrix}$$

4.  Évaluation du modèle

Nous présentons les résultats du modèle dans la section *Analyse, discussion et conclusion*.
