...
À noter que, bien que [Chaduri et al., 2021](@chaudhuri2021inundated) fait ses prédictions sur la médiane, nous nous concentrerons sur la moyenne des débits annuels. La raison étant que les modèles mixtes standards sont fait pour estimer la moyenne d'une distribution normale et que les données de RiverAtlas ne fournissent que cette information et non pas les quantiles ou toutes les observations historiques.
...
D'une part, notons que chaque variable est mesurée pour le bassin observé ( _cav_ ) et pour les bassins en amont ( _uav_ ). Or, dans ces cas, le patron de non-réponse est très similaire. En effet, si une variable est manquante pour tous les bassins en amont, elle le sera assurément pour le bassin présent. Par ailleurs, notamment grâce au Tableau 2.1.2 (Annexe A1), il est intéressant d'observer que les variables _cav_ et _uav_ sont linéairement très corrélées.
...
En utilisant la formule `naniar::mcar_test`, nous obtenons que le seuil observé est de 0 qui confirme notre hypothèse. On rejette, donc, l'hypothèse $H_0 :$ Données manquantes avec un mécanisme _MCAR_.
...
L'EQM obtenu pour notre jeu de test et, en comparaison, notre jeu d'entraînement :
...
$$H_0 = \text{Modèle simple}$$
$$H_1 = \text{Modèle plus complexe}$$
Les modèles testés sont les suivants :

1. Modèle avec une ordonnée à l'origine aléatoire
2. Modèle avec une ordonnée à l'origine aléatoire et une pente aléatoire pour la variable _ria_ha_csu_ (sans corrélation, ce qui implique que la matrice de variance-covariance est diagonale)
3. Modèle avec une ordonnée à l'origine aléatoire et une pente aléatoire pour la variable _ria_ha_csu_ (avec corrélation, chaque combinaison d'effets aléatoires a un coefficient de corrélation différent)
4. Modèle avec une ordonnée à l'origine aléatoire et des pentes aléatoires pour les variables _ria_ha_csu_ et _riv_tc_usu_ (sans corrélation)
5. Modèle avec une ordonnée à l'origine aléatoire et des pentes aléatoires pour les variables _ria_ha_csu_ et _riv_tc_usu_ (avec corrélation)
...
- $\beta_1$ est lié la pente de la variable _ria_ha_csu_
- $\beta_2$ est lié à la pente de la variable _riv_tc_usu_

Ces effets aléatoires ont une matrice de variance-covariance telle que : 
$$\begin{bmatrix}
...
Nous présentons les résultats du modèle dans la section _Analyse, discussion et conclusion_.
